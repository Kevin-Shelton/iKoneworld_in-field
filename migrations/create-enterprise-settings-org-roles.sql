-- Migration: Create enterprise_settings table with organization roles
-- Purpose: Enable admin-level controls for audio recordings and transcripts
-- Date: 2024-11-04
-- Note: Uses organization-aligned roles (enterprise_admin, regional_director, etc.)
...
USING (
  enterprise_id IN (
    SELECT enterprise_id FROM users WHERE "openId" = auth.uid()
  )
);
...
BEGIN
  ...
  WHERE enterprise_id = '00000000-0000-00';
