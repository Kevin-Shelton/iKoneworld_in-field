...
import { translatePDFWithDeepL } from '@/lib/deeplPdfTranslator';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Process PDF] Starting background translation for conversation ${conversationId}`);
    ...
    if (fetchError || !conversation) {
      console.error('[Process PDF] Failed to fetch conversation:', fetchError);
    ...
    console.log(`[Process PDF] Translating PDF with DeepL (${sourceLanguage} → ${targetLanguage})`);
    
    // 5. Translate PDF with DeepL
    if (!process.env.DEEPL_API_KEY) {
      console.error('[Process PDF] DeepL API key not configured');
      await supabase.from('conversations').update({
        status: 'failed',
        metadata: {
          ...metadata,
    ...
    console.log(`[Process PDF] Translation completed, uploading translated PDF`);
    
    // 6. Generate translated filename
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const originalName = (originalFilename || 'document.pdf').replace('.pdf', '');
    const newFilename = `${originalName}_${sourceLanguage.toUpperCase()}_to_${targetLanguage.toUpperCase()}_${timestamp}.pdf`;
    
    // 7. Upload translated PDF to Supabase storage
    ...
    console.log(`[Process PDF] Translation completed in ${Math.round(processingDurationMs / 1000)}s`);
    ...
    console.error('[Process PDF] Translation error:', error);
    ...
    try {
      ...
      console.error('[Process PDF] Failed to update error status:', updateError);
    }
