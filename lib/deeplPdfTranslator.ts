/**
 * DeepL PDF Document Translator
 * 
 * Uses DeepL's Document Translation API to translate PDFs while preserving
 * all formatting, images, colors, fonts, and layout.
 */

import * as deepl from 'deepl-node';

/**
 * Translate a PDF document using DeepL's Document Translation API
 * 
...
export async function translatePDFWithDeepL(
  ...
  console.log('[DeepL] Starting PDF translation');
  console.log('[DeepL] File:', fileName);
  console.log('[DeepL] Source language:', sourceLanguage);
  console.log('[DeepL] Target language:', targetLanguage);
  console.log('[DeepL] File size:', fileBuffer.length, 'bytes');
  ...
  try {
    ...
    console.log('[DeepL] Uploading document...');
    ...
    console.log('[DeepL] Document uploaded successfully');
    console.log('[DeepL] Document ID:', uploadResult.documentId);

    // Step 2: Poll for translation completion
    console.log('[DeepL] Waiting for translation to complete...');
    ...
    while (status.status !== 'done' && (Date.now() - startTime) < maxWaitTime) {
      ...
      console.log('[DeepL] Translation status:', status.status, 
    ...
    console.log('[DeepL] Billed characters:', status.billedCharacters);

    // Step 3: Download translated document
    console.log('[DeepL] Downloading translated document...');
    ...
    console.log('[DeepL] Downloaded translated document, size:', translatedBuffer.length, 'bytes');
    
    // Clean up temp file
    try {
      await fs.unlink(tempFile);
    } catch (e) {
      console.warn('[DeepL] Failed to delete temp file:', tempFile);
    }
